...
- **`main`** — the REPL loop: rescan, redraw, prompt for a menu choice (`N°` to launch a CLI interactively, `A` to ask the orchestrator agent, `M` to configure per-CLI models, `R` to refresh, `Q` to quit).
